...
// Replace rsr-template-repo, anytype, {{DEPS}} with actuals
...
| `$PREFIX/share/anytype/`
| Data files, assets

| `$PREFIX/share/doc/anytype/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/anytype/config.toml`

Fallback: `$HOME/.config/anytype/config.toml`
...
Or via OPSM: `opsm update anytype`
...
[source,bash]
----
just install --prefix=/opt/anytype-instance1 --config=/etc/anytype/instance1.toml
just install --prefix=/opt/anytype-instance2 --config=/etc/anytype/instance2.toml
----
